...
export default function JobDetail({ job, onClose, onEdit, onDelete }: Props) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div
        className="animate-slide-in fixed top-0 right-0 h-full flex flex-col border-l border-white/5"
        style={{
          background: '#13151f',
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          <div className="flex flex-col rounded-xl overflow-hidden border border-white/5 mb-5">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5" style={{ background: '#1a1d27' }}>
              ...
              <span className="text-xs text-gray-300">
                {new Date(job.applied_date).toLocaleDateString('da-DK', { day: 'numeric', month: 'long', year: 'numeric' })}
              </span>
            </div>
            {job.job_url && (
              <div className="flex items-center justify-between px-4 py-3" style={{ background: '#1a1d27' }}>
                <span className="text-xs text-gray-500">Link</span>
                <a href={job.job_url} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">
                  Abn jobopslag
                </a>
              </div>
            )}
          ...
          {job.notes && (
            <div className="mb-5">
              ...
              <div className="rounded-xl px-4 py-3 text-sm text-gray-400 leading-relaxed border border-white/5" style={{ background: '#1a1d27' }}>
                {job.notes}
              </div>
            </div>
          )}
        ...
        <div className="px-5 py-4 border-t border-white/5 flex gap-2">
          <button onClick={() => onEdit(job)} className="flex-1 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-400 border border-indigo-500/20 rounded-lg py-2 text-sm font-medium transition-colors">
            Rediger
          </button>
          <button onClick={() => onDelete(job.id)} className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-lg py-2 text-sm font-medium transition-colors">
            Slet
          </button>
        </div>
      </div>
